...
void FINISH(void){
	reset();
	draw_formatted(screen_width()/2,screen_height()/2, "CONGRATULATIONS! \n Your final score was:%2d \n Press any key to exit.", timerMin, timerSec, lvlNum, lives, score);
...
void move_platformLVL5B( void ) {
	...
	int y = round(sprite_y(platformLVL5B));
...
void WORLD_BARRIER(void){
	...
	int hr = x + HERO_WIDTH - 1;
	int hb = y + HERO_HEIGHT - 1;
...
void LEVEL4_BARRIER2(void){
	...
	int xplatOne = round(sprite_x(platformOne));
	int yplatOne = round(sprite_y(platformOne));
	int platEND = platPOSX + platformLength;
	int hr = x + HERO_WIDTH - 1;
	int hb = y + HERO_HEIGHT - 1;
